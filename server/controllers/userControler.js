...
const loginController = async(req, res) => {
    try {
        ...
        return res.status(200).json({success: true, message: `Welcome Back, ${user.name}`, token, user: {_id: user._id, name: user.name, role: user.role}});
    } catch (error) {
...
const verifyController = (req, res) => {
    return res.status(200).json({success: true, message: "Verified user fetched successfully", user: req.user})
}
