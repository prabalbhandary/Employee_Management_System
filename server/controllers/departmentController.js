...
const getDepartments = async (req, res) => {
    try {
        ...
        return res.status(200).json({success: true, message: "Departments fetched successfully", departments});
    } catch (error) {
        return res.status(500).json({success: false, message: "Internal Server Error"});
    }
};
...
const getDepartmentById = async(req, res) => {
    try {
        ...
        return res.status(200).json({success: true, message: "Department fetched successfully", department});
    } catch (error) {
        return res.status(500).json({success: false, message: "Internal Server Error"});
    }
}
...
const editDepartment = async(req, res) => {
    try {
        ...
        return res.status(200).json({success: true, message: "Department updated successfully", updatedDep});
    } catch (error) {
        return res.status(500).json({success: false, message: "Internal Server Error"});
    }
}
